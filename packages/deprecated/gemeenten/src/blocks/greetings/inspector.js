...
function Inspector( props ) {
    ...
    const [newRange, setNewRange] = useState({ start: '', end: '', value: '' });
    ...
    const isValidRange = (start, end) => {
        ...
        for (const key in greetings) {
            const [existingStart, existingEnd] = key.split('-').map(time => parseInt(time.replace(':', ''), 10));
    ...
    const addRange = () => {
        if (!isValidRange(newRange.start, newRange.end)) {
            setError(__('De tijdsperiode overlapt met een bestaande periode.'));
            return;
        }
        const key = `${newRange.start}-${newRange.end}`;
        setAttributes({ greetings: { ...greetings, [key]: newRange.value } });
        ...
        <InspectorControls>
            <PanelBody title={ __( 'Opties' ) }>
                ...
                {Object.keys(greetings).map((key) => (
                    <div key={key} style={{ marginBottom: '10px' }}>
                        <TextControl
                            label={ `${key} uur` }
                            value={ greetings[key] }
                            onChange={ (value) => setGreetingKey(value, key) }
                        />
                        <Button
                            isDestructive
                ...
                <TextControl
                    ...
                    onChange={ (value) => setNewRange({ ...newRange, start: value }) }
                />
                <TextControl
                    label={ __( 'Eind Tijd (HH:MM)' ) }
                    value={ newRange.end }
                    onChange={ (value) => setNewRange({ ...newRange, end: value }) }
                />
                <TextControl
                    label={ __( 'Label' ) }
                    value={ newRange.value }
                    onChange={ (value) => setNewRange({ ...newRange, value: value }) }
                />
                <Button
                    isPrimary
                    onClick={ addRange }
