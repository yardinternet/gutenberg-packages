...
const Label = styled.label`
	...
	transform: translate(var(--pdc-search-padding), calc(var(--pdc-search-padding) / 2)) scale(1);
	transition: color 200ms cubic-bezier(0, 0, 0.2, 1) 0ms,
		transform 200ms cubic-bezier(0, 0, 0.2, 1) 0ms;

	&.gemeenten-block--pdc-search__label--active {
		background: #fff;
		transform: translate(var(--pdc-search-padding), calc(var(--pdc-search-padding) / -2.5))
			scale(0.8);
...
const Btn = styled.button`
	...
	border-radius: 0 var(--pdc-search-border-radius) var(--pdc-search-border-radius) 0;
	box-shadow: var(--pdc-search-box-shadow);
	color: white;
	font-size: var(--pdc-search-font-size);
	...
	@media (max-width: ${minWidthBreakpoint}) {
		padding: calc(var(--pdc-search-padding) / 2) calc(var(--pdc-search-padding) / 1.2);
		i {
			font-size: 1.2rem;
		}
	}
...
function BasicForm(props) {
	...
	const onChange = (event) => {
		...
		return event.target.value.length ? setLabelClasses(labelShrinkClass) : setLabelClasses('');
	};
	...
	return (
		<Wrapper className="gemeenten-block gemeenten-block--pdc-search">
			...
			>
				<Label
					className={ classnames('gemeenten-block--pdc-search__label', labelClasses) }
					htmlFor="pdc-search"
				>
					{ props.label }
				</Label>
