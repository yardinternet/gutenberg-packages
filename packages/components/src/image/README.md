...
Easily add images to custom blocks without needing to manually worry about loading states. It renders a `<MediaPlaceholder />` component in place of the image if the id is not set and shows a spinner when the image is still loading.
    ...
    return (
        <Image
            ...
            labels={{
                ...
                instructions: 'Upload een afbeelding of kies er één uit de mediabibliotheek.'
            }}
        />
    )
}
...
> **Note**
> Recommended to add the `MediaToolbar` component. It adds the Replace flow to the Blocks Toolbar.
