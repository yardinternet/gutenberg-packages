			...
			if ( currentBlock && !! currentBlock.innerBlocks ) {
				currentBlock.innerBlocks.map( ( innerBlock ) => {
					return updateBlockAttributes( innerBlock.clientId, {
						...
						...{ parentCardCount: cardsPerRow > 0 ? cardsPerRow : newBlockCount },
					} );
				} );
			}

			updateBlockAttributes( clientId, { ...attributes, ...{ cardCount: newBlockCount } } );
		}
	} );
		...
		<>
			...
			<Cards { ...props }>
				<InnerBlocks
					...
					renderAppender={ () => (
						<BasicBlockAppender clientId={ clientId } blockName={ cardMetaData.name } />
					) }
				/>
			</Cards>
		</>
